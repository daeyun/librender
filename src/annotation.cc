...
Annotation::Annotation(float grid_z) {
    ...
    if (config::are_axes_visible) {
        this->axis_data = new ScryObject();
        this->axis_data->type = kLines;
        this->axis_data->v.reshape(3, 6);
        this->axis_data->f.reshape(2, 3);
        this->axis_data->cdata.reshape(4, 6);
        this->axis_data->cdata.each_col() = arma::fvec4({0.5, 0.5, 0.5, 1});
        ...
        this->gl_axes = GLObject(this->axis_data, simple_shader, NULL);
    }

    length = 1.5;
    float spacing = 0.1;

    this->grid_data = new ScryObject();
    ...
    this->grid_data->cdata.reshape(4, int(2*length/spacing+1)*4);
    this->grid_data->cdata.each_col() = arma::fvec4({0.8, 0.8, 0.8, 1});
    ...
    this->gl_grid = GLObject(this->grid_data, simple_shader, NULL);
}
